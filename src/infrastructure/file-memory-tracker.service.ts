...
export class FileMemoryTrackerService {
  ...
  async trackMemory(
    ...
    if (newTracker.isKo()) {
      throw new Error('Invalid FileMemoryTracker: ' + newTracker.getError().message);
    }

    const tracker = await this.repository.findOrCreate(newTracker.getValue());

    // Use aggregate business logic for remember
    const remembered = tracker.remember(memoryId);
    if (remembered.isKo()) {
      throw new Error('Failed to remember memory: ' + remembered.getError().message);
    }
  ...
  async forgetMemory(filePath: string, memoryId: string): Promise<FileMemoryTracker | null> {
    const tracker = await this.repository.findByFilePath(filePath);
    if (!tracker) {
      return null;
    }
    ...
    if (forgotten.isKo()) {
      throw new Error('Failed to forget memory: ' + forgotten.getError().message);
    }
