...
describe('TextChunker', () => {
  ...
  it('should split large text at sentence boundaries', async () => {
    const sentences = Array.from(
      { length: 50 },
      (_, i) => `This is sentence number ${i + 1} with additional words to increase token count.`,
    ).join(' ');
  ...
  it('should track chunkIndex and totalChunks correctly', async () => {
    const sentences = Array.from(
      { length: 50 },
      (_, i) => `This is sentence number ${i + 1} with more words to ensure token count.`,
    ).join(' ');
  ...
  it('should respect maxTokens and not exceed hardCapTokens', async () => {
    const sentences = Array.from(
      { length: 30 },
      (_, i) => `This is a longer sentence number ${i + 1} with more words.`,
    ).join(' ');
