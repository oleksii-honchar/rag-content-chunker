...
export class CodeChunker implements Chunker {
  ...
  private recursiveChunk(
    ...
    return [
      ...
      { text: secondPart.trim(), startLine: firstEndLine + 1, endLine: firstEndLine + secondPart.split('\n').length },
    ];
  }
  ...
  private trySplitBySeparator(
    ...
    if (separator === '' || !text.includes(separator)) {
      return [{ text: text.trim(), startLine: baseStartLine, endLine: baseStartLine + text.split('\n').length - 1 }];
    }
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (currentChunkText && combinedTokens > config.maxTokens && estimateTokens(currentChunkText) > config.maxTokens / 2) {
