...
describe('IngestChunkUseCase', () => {
  ...
  describe('execute', () => {
    ...
    it('should log success count per chunk', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith('Chunk ingested', expect.objectContaining({
        chunkId: chunks[0].id,
        chunkIndex: 0,
      }));
    });
    ...
    it('should log failure count per chunk', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Chunk ingestion failed', expect.objectContaining({
        chunkId: chunk.id,
        error: 'MCP error',
      }));
    });
    ...
    it('should log completion summary with success/failure counts', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Chunk ingestion completed', expect.objectContaining({
        sourceId: 'test-source',
        totalChunks: 2,
        successCount: 1,
        failureCount: 1,
